...
========================================================= */
  ...
  const nowLine  = qs('#nowLine') || null; // guard
  const addBtn   = qs('#addBtn');
  const wrap     = qs('#calWrap');

/* =========================================================
   D) LAYOUT: Sticky/Viewport helpers
========================================================= */
...
/* =========================================================
   F) WEEK HEADER (weekday labels)
========================================================= */
  function applyWeekHeader(){
    for(let i=1;i<=7;i++){
      const dISO = addDays(weekStartISO, i-1);
      void new Date(dISO+"T00:00:00Z"); // d is unused (text only)
...
/* =========================================================
   G) EVENT LAYOUT (overlap → columns)
========================================================= */
...
/* =========================================================
   I) NOW LINE (current time indicator)
========================================================= */
...
/* =========================================================
   J) MODAL EDITOR (fill/reset/open)
========================================================= */
...
/* =========================================================
   K) USER INTERACTIONS (dblclick create, add button)
========================================================= */
...
========================================================= */
  qs('#saveBtn').addEventListener('click', async ()=>{
    try{
      ...
      if(!start || !end){ showInfo("error", "Լրացրեք սկիզբը և ավարտի ժամը!", ""); return; }
      if((toMinutesSafe(end) ?? 0) <= (toMinutesSafe(start) ?? 0)){ showInfo("error", "Ավարտի ժամը պետք է լինի ավելի ուշ, քան սկիզբը!", ""); return; }
...
/* =========================================================
   O) DATA LOAD (fetch + normalize + render)
========================================================= */
...
========================================================= */
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // modal full reset on close
    const $modal = $('#eventModal');
    $modal.on('hidden.bs.modal', function(){
      resetModalHard($modal);
    });

    // react on filter changes (jQuery zone dispatches)
...
$(document).ready(function(){
  ...
  // --- Dependent selects inside modal ---
